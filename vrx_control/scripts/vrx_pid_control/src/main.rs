...
rosrust::rosmsg_include!(std_msgs/Float64, geometry_msgs/Twist, nav_msgs/Odometry);
...
fn main() {
    ...
    let left_thruster_publisher = rosrust::publish::<std_msgs::Float64>("/wamv/thrusters/left_thrust_cmd", 100).unwrap();
    let right_thruster_publisher = rosrust::publish::<std_msgs::Float64>("/wamv/thrusters/right_thrust_cmd", 100).unwrap();
    ...
    let cmd_vel_subscriber = rosrust::subscribe("/cmd_vel", 100, move |msg: geometry_msgs::Twist| {
        let req_linear_vel: f64 = msg.linear.x;
        let req_angular_vel: f64 = msg.angular.z;
    ...
    let odom_subscriber = rosrust::subscribe("/odometry/filtered/local", 100, move |msg: nav_msgs::Odometry| {
        let linear_vel: f64 = msg.twist.twist.linear.x;
        let angular_vel: f64 = msg.twist.twist.angular.z;
    ...
    let left_control_effort_subscriber = rosrust::subscribe("/left_thruster_controller/control_effort", 100, move |msg: std_msgs::Float64| {
        left_thruster_publisher.send(msg).unwrap();
    }).unwrap();
    ...
    let right_control_effort_subscriber = rosrust::subscribe("/right_thruster_controller/control_effort", 100, move |msg: std_msgs::Float64| {
        right_thruster_publisher.send(msg).unwrap();
    }).unwrap();
